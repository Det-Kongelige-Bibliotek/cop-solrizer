...
<xsl:transform xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 <xsl:template match="/">
  ...
  <xsl:template name="field_name">
	...
	<xsl:value-of select="concat($ns,$name,'_en')"/>
      </xsl:when>
      <xsl:when test="@xml:lang = 'da' or  $lang = 'da'">
      	<xsl:value-of select="concat($ns,$name,'_da')"/>
      </xsl:when>
      <xsl:when test="contains(@xml:lang,'he') or  contains($lang,'he')">
      	<xsl:value-of select="concat($ns,$name,'_he')"/>
      </xsl:when>
      <xsl:otherwise>
      	<xsl:value-of select="concat($ns,$name,'_txt')"/>
      </xsl:otherwise>
    </xsl:choose>
  </xsl:template>
  ...
  <xsl:template match="ese:record">
    ...
    <xsl:element name="field">
      <xsl:attribute name="name">title_display</xsl:attribute>
	...
	<xsl:value-of select="normalize-space(.)"/>
      ...
      <xsl:element name="field">
	<xsl:attribute name="name">author_display</xsl:attribute><xsl:if test="position() &lt; last()"><xsl:text>; </xsl:text></xsl:if>
    ...
    <xsl:for-each select="dc:description|dc:format|dc:type|dc:language|dc:contributor|dc:publisher|dc:rights|dc:coverage">
      <xsl:element name="field">
	<xsl:attribute name="name"><xsl:value-of select="concat(local-name(.),'_t')"/></xsl:attribute>
	<xsl:value-of select="normalize-space(.)"/>
      </xsl:element>
    </xsl:for-each>
    ...
    <xsl:for-each select="dc:date[number(.) = number(.)][1]">
      <xsl:element name="field">
	<xsl:attribute name="name">pub_date</xsl:attribute>
	<xsl:value-of select="number(.)" /> <!-- concat(normalize-space(.),'-12-31T23:59:59Z')"/ -->
      </xsl:element>
    </xsl:for-each>
    <xsl:for-each select="dc:date">
      <xsl:element name="field">
	<xsl:attribute name="name">readable_date_string_t</xsl:attribute>
	<xsl:value-of select="normalize-space(.)"/>
      </xsl:element>
    </xsl:for-each>
	...
	<xsl:for-each  select="h:a[not(contains(@href,'/editions/'))]">
	  <xsl:element name="field">
		...
		<xsl:attribute name="name"><xsl:value-of select="concat('subject_',@xml:lang)"/></xsl:attribute>
	      ...
	      <xsl:otherwise -->
		<xsl:attribute name="name">subject_topic_facet</xsl:attribute>
	      <!-- /xsl:otherwise>
	    </xsl:choose -->
	    <xsl:value-of select="normalize-space(.)"/>
	</xsl:element>
    ...
    <xsl:for-each select="ese:type|ese:rights|ese:dataProvider">
      <xsl:element name="field">
	<xsl:attribute name="name">ese_<xsl:value-of select="local-name(.)"/>_t</xsl:attribute>
	<xsl:value-of select="normalize-space(.)"/>
      </xsl:element>
    </xsl:for-each>
	...
	<xsl:value-of select="normalize-space(.)"/>
     ...
     <xsl:for-each select="ese:isShownAt">
      <xsl:element name="field">
	<xsl:attribute name="name">mods_t</xsl:attribute>
